...
export function AiChat() {
  ...
  const speakResponse = (text: string, lang: 'en' | 'ml') => {
    ...
    if (lang === 'ml') {
      ...
      fetch(`https://translate.google.com/translate_tts?ie=UTF-8&q=${encodeURIComponent(text)}&tl=ml&client=tw-ob`, {
        headers: {
      ...
      // Use speechSynthesis for English
      if (!synth) return;
      const utterance = new SpeechSynthesisUtterance(text);
      utteranceRef.current = utterance;
      utterance.lang = 'en-US';
      const voices = synth.getVoices();
      const voice = voices.find((v) => v.lang.toLowerCase().includes('en')) || voices.find((v) => v.lang.startsWith('en'));
